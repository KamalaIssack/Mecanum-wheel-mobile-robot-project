...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, M1_REN_Pin|M1_LEN_Pin|M2_REN_Pin|M2_LEN_Pin
                          |M3_REN_Pin|M3_LEN_Pin|M4_REN_Pin|M4_LEN_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : M1_REN_Pin M1_LEN_Pin M2_REN_Pin M2_LEN_Pin
                           M3_REN_Pin M3_LEN_Pin M4_REN_Pin M4_LEN_Pin */
  GPIO_InitStruct.Pin = M1_REN_Pin|M1_LEN_Pin|M2_REN_Pin|M2_LEN_Pin
                          |M3_REN_Pin|M3_LEN_Pin|M4_REN_Pin|M4_LEN_Pin;
               ...
               int16_t speed)
{
    ...
    if (speed > 0)
    {
        __HAL_TIM_SET_COMPARE(htim, rpwm_ch, duty);
        __HAL_TIM_SET_COMPARE(htim, lpwm_ch, 0);
    }
    else if (speed < 0)
    {
        __HAL_TIM_SET_COMPARE(htim, rpwm_ch, 0);
        __HAL_TIM_SET_COMPARE(htim, lpwm_ch, duty);
    }
    else
    {
        __HAL_TIM_SET_COMPARE(htim, rpwm_ch, 0);
...
void mecanum_drive(int16_t vx, int16_t vy, int16_t omega)
{
    int16_t fl = vx - vy - omega;
    int16_t fr = vx + vy + omega;
    int16_t rl = vx + vy - omega;
    int16_t rr = vx - vy + omega;
